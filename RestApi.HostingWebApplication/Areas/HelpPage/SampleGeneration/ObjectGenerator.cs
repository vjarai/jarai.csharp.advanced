...
namespace Jarai.RestApi.HostingWebApplication.Areas.HelpPage.SampleGeneration
{
    ...
    public class ObjectGenerator
    {
        ...
        private static object GenerateGenericType(Type type, int collectionSize, Dictionary<Type, object> createdObjectReferences)
        {
            ...
            if (genericTypeDefinition == typeof(KeyValuePair<,>)) return GenerateKeyValuePair(type, createdObjectReferences);
            ...
            if (genericArguments.Length == 1)
            {
                ...
                if (genericTypeDefinition == typeof(IQueryable<>)) return GenerateQueryable(type, collectionSize, createdObjectReferences);

                var closedCollectionType = typeof(ICollection<>).MakeGenericType(genericArguments[0]);
                if (closedCollectionType.IsAssignableFrom(type)) return GenerateCollection(type, collectionSize, createdObjectReferences);
            }
            ...
            if (genericArguments.Length == 2)
            {
                ...
                if (closedDictionaryType.IsAssignableFrom(type)) return GenerateDictionary(type, collectionSize, createdObjectReferences);
            }
        ...
        private object GenerateObject(Type type, Dictionary<Type, object> createdObjectReferences)
        {
            try
            {
                if (SimpleTypeObjectGenerator.CanGenerateObject(type)) return SimpleObjectGenerator.GenerateObject(type);

                if (type.IsArray) return GenerateArray(type, DefaultCollectionSize, createdObjectReferences);

                if (type.IsGenericType) return GenerateGenericType(type, DefaultCollectionSize, createdObjectReferences);
        ...
        private class SimpleTypeObjectGenerator
        {
            ...
            private static Dictionary<Type, Func<long, object>> InitializeGenerators()
            {
                return new Dictionary<Type, Func<long, object>>
                {
                    ...
                    {
                        typeof(Uri),
                        index => { return new Uri(string.Format(CultureInfo.CurrentCulture, "http://webapihelppage{0}.com", index)); }
                    }
                };
            }
        }
